...
<html lang="en">
<head>
    <title>Detection D Immatriculation</title>
    <meta charset="utf-8">
          ...
          <div class="grid_12">
            <div class="wrap pad-3">
                <div class="block-5" style="width:900px;">
                    <h3>Systeme De Reconnaissance Des Plaques D Immatriculation</h3>
                       <div class="map img-border" style="width:900px;">
                        {% if frame %}
                            <img src="/static/generated_frames_valid/{{frame}}" width='890' height='182' class="img-polaroid" style="width:890px;height:182px;"/>
                        {% else %}
                            <img src="/static/nlpImg/car.jpeg" width='890' height='182' class="img-polaroid" style="width:890px;height:182px;"/>
                       {% endif %}
                        </div>
                       <form id="form" method="post" action="/recognizePlate/">
                      <fieldset>
                        <div class="btns">
                            <input type="hidden" value="{{frame}}" name="frame"/>
                            <input type="submit" value="Reconnaitre" class="button" onClick="document.getElementById('form').submit()" style="width:100px;height:30px; background-color:rgb(0,138,138);"/>
                             {% if res %} <div>Immatriculation Found : {{ res }}</div>{% endif %}
                        </div>
                      </fieldset>
                      </form>
                </div>
